...
outputDir = r'E:\Projects\Clients\Nashville\Tasks\Task2_PopSim\Setup\data'
...
if not os.path.isfile(os.path.join(censusDownloadDir, 'hhsize_BG_2017_acs5.csv')):	
	hhsize_BG = pd.DataFrame()
	for i,cen in enumerate(COUNTY_FIPS_STRING):	
		if i == 0:
			hhsize_BG = getCensusData(name = "acs/acs5", 	
									 vintage = '2017', 	
									 key = api_key, 	
			...
			hhsize_BG = hhsize_BG.append(getCensusData(name = "acs/acs5", 	
									 vintage = '2017', 	
									 key = api_key, 	
									 vars="B11016_001E,B11016_002E,B11016_003E,B11016_004E,B11016_005E,B11016_006E,B11016_007E,B11016_008E,B11016_009E,B11016_010E,B11016_011E,B11016_012E,B11016_013E,B11016_014E,B11016_015E,B11016_016E",
									 region = "block%20group:*",	
	...
	hhsize_BG.to_csv(os.path.join(censusDownloadDir, 'hhsize_BG_2017_acs5.csv'), index = False)

if not os.path.isfile(os.path.join(censusDownloadDir, 'hhtype_BG_2017_acs5.csv')):	
	...
	for i,cen in enumerate(COUNTY_FIPS_STRING):	
		if i == 0:
			...
			hhtype_BG = hhtype_BG.append(getCensusData(name = "acs/acs5", 	
									 vintage = '2017', 	
									 key = api_key, 	
									 vars="B11001_003E,B11001_005E,B11001_006E,B11001_008E,B11001_009E",
									 region = "block%20group:*",	
	...
	hhtype_BG.to_csv(os.path.join(censusDownloadDir, 'hhtype_BG_2017_acs5.csv'), index = False)

#B26103_001E,B26103_006E,B26103_007E - total gqs, total non-inst gqs, college/univ gqs
if not os.path.isfile(os.path.join(censusDownloadDir, 'gqtot_CT_2017_acs5.csv')):
	gqs_BG = pd.DataFrame()
	for i,cen in enumerate(COUNTY_FIPS_STRING):	
		if i == 0:
			...
			gqs_BG = gqs_BG.append(getCensusData(name = "acs/acs5", 	
									 vintage = '2017', 	
									 key = api_key, 	
									 vars="B26001_001E", 
									 region = "tract:*",	
	...
	gqs_BG.to_csv(os.path.join(censusDownloadDir, 'gqtot_CT_2017_acs5.csv'), index = False)
...
if not os.path.isfile(os.path.join(censusDownloadDir, 'gqage_CT_2010_sf1.csv')):
	...
	for i,cen in enumerate(COUNTY_FIPS_STRING):	
		if i == 0: 
			...
			gqs_BG = gqs_BG.append(getCensusData(name = "dec/sf1", 	
									 vintage = '2010', 	
									 key = api_key, 	
									 vars="P043001,P043002,P043003,P043013,P043023,P043033,P043034,P043044,P043054", 
									 region = "tract:*",	
...
if not os.path.isfile(os.path.join(censusDownloadDir, 'hhtenure_BG_2017_acs5.csv')):
	...
	for i,cen in enumerate(COUNTY_FIPS_STRING):	
		if i == 0:
			...
			hhtenure_BG = hhtenure_BG.append(getCensusData(name = "acs/acs5", 	
									 vintage = '2017', 	
									 key = api_key, 	
									 vars="B25007_001E,B25007_002E,B25007_003E,B25007_004E,B25007_005E,B25007_006E,B25007_007E,B25007_008E,B25007_009E,B25007_010E,B25007_011E,B25007_012E,B25007_013E,B25007_014E,B25007_015E,B25007_016E,B25007_017E,B25007_018E,B25007_019E,B25007_020E,B25007_021E", 
									 region = "block%20group:*",	
	...
	hhtenure_BG.to_csv(os.path.join(censusDownloadDir, 'hhtenure_BG_2017_acs5.csv'), index = False)
	
	
if not os.path.isfile(os.path.join(censusDownloadDir, 'hhworker_CT_2017_acs5.csv')):
	...
	for i,cen in enumerate(COUNTY_FIPS_STRING):	
		if i == 0:
			...
			hhworker_CT = hhworker_CT.append(getCensusData(name = "acs/acs5", 	
									 vintage = '2017', 	
									 key = api_key, 	
									 vars="B08202_001E,B08202_002E,B08202_003E,B08202_004E,B08202_005E", 	
									 region = "tract:*",	
	...
	hhworker_CT.to_csv(os.path.join(censusDownloadDir, 'hhworker_CT_2017_acs5.csv'), index = False)

if not os.path.isfile(os.path.join(censusDownloadDir, 'hhincome_CT_2017_acs5.csv')):
	...
	for i,cen in enumerate(COUNTY_FIPS_STRING):	
		if i == 0:
			...
			hhincome_CT = hhincome_CT.append(getCensusData(name = "acs/acs5", 	
									 vintage = '2017', 	
									 key = api_key, 	
									 vars="B19001_001E,B19001_002E,B19001_003E,B19001_004E,B19001_005E,B19001_006E,B19001_007E,B19001_008E,B19001_009E,B19001_010E,B19001_011E,B19001_012E,B19001_013E,B19001_014E,B19001_015E,B19001_016E,B19001_017E",	
									 region = "tract:*",	
	...
	hhincome_CT.to_csv(os.path.join(censusDownloadDir, 'hhincome_CT_2017_acs5.csv'), index = False)

if not os.path.isfile(os.path.join(censusDownloadDir, 'populationbyageandsex_CT_2017_acs5.csv')):
	...
	for i,cen in enumerate(COUNTY_FIPS_STRING):	
		if i == 0:
			...
			popage_CT = popage_CT.append(getCensusData(name = "acs/acs5", 	
									 vintage = '2017', 	
									 key = api_key, 	
									 vars="B01001_001E,B01001_002E,B01001_003E,B01001_004E,B01001_005E,B01001_006E,B01001_007E,B01001_008E,B01001_009E,B01001_010E,B01001_011E,B01001_012E,B01001_013E,B01001_014E,B01001_015E,B01001_016E,B01001_017E,B01001_018E,B01001_019E,B01001_020E,B01001_021E,B01001_022E,B01001_023E,B01001_024E,B01001_025E,B01001_026E,B01001_027E,B01001_028E,B01001_029E,B01001_030E,B01001_031E,B01001_032E,B01001_033E,B01001_034E,B01001_035E,B01001_036E,B01001_037E,B01001_038E,B01001_039E,B01001_040E,B01001_041E,B01001_042E,B01001_043E,B01001_044E,B01001_045E,B01001_046E,B01001_047E,B01001_048E,B01001_049E", 	
									 region = "tract:*",	
	...
	popage_CT.to_csv(os.path.join(censusDownloadDir, 'populationbyageandsex_CT_2017_acs5.csv'), index = False)

if not os.path.isfile(os.path.join(censusDownloadDir, 'hh_kids_nokids_CT_2017_acs5.csv')):
	...
	for i,cen in enumerate(COUNTY_FIPS_STRING):	
		if i == 0:
			...
			hhkids_CT = hhkids_CT.append(getCensusData(name = "acs/acs5", 	
									 vintage = '2017', 	
									 key = api_key, 	
									 vars="B25012_001E,B25012_002E,B25012_003E,B25012_004E,B25012_005E,B25012_006E,B25012_007E,B25012_008E,B25012_009E,B25012_010E,B25012_011E,B25012_012E,B25012_013E,B25012_014E,B25012_015E,B25012_016E,B25012_017E",
									 region = "tract:*",	
	...
	hhkids_CT.to_csv(os.path.join(censusDownloadDir, 'hh_kids_nokids_CT_2017_acs5.csv'), index = False)

if not os.path.isfile(os.path.join(censusDownloadDir, 'hhunit_CT_2017_acs5.csv')):
        #list of variables: https://api.census.gov/data/2017/acs/acs5/variables.html
	...
	for i,cen in enumerate(COUNTY_FIPS_STRING):	
		if i == 0:
			...
			hhunit_CT = hhunit_CT.append(getCensusData(name = "acs/acs5", 	
									 vintage = '2017', 	
									 key = api_key, 	
									 vars="B11011_004E,B11011_005E,B11011_006E,B11011_009E,B11011_010E,B11011_011E,B11011_013E,B11011_014E,B11011_015E,B11011_017E,B11011_018E,B11011_019E",
									 region = "tract:*",	
	...
	hhunit_CT.to_csv(os.path.join(censusDownloadDir, 'hhunit_CT_2017_acs5.csv'), index = False)


# if not os.path.isfile(os.path.join(censusDownloadDir, 'GQPop_CT_2010_sf1.csv')):
        # list of variables: https://api.census.gov/data/2017/acs/acs5/variables.html
	# gqpop_CT = pd.DataFrame()
	# for i,cen in enumerate(COUNTY_FIPS_STRING):	
		# if i == 0:
			# gqpop_CT = getCensusData(name = "sf1", 	
									 ...
									 # regionin = "state:{}%20county:{}".format(STATE_FIPS, cen))
		# else:
			# gqpop_CT = gqpop_CT.append(getCensusData(name = "sf1", 	
									 # vintage = '2010', 	
									 # key = api_key, 	
									 # vars="P043001,P043002,P043003,P043013,P043023,P043033,P043034,P043044,P043054", 	
									 # region = "tract:*",	
	...
	# gqpop_CT.to_csv(os.path.join(censusDownloadDir, 'gqpop_CT_2010_sf1.csv'), index = False)
