...
popSimDir       <- trimws(paste(parameters$Value[parameters$Key=="POPSIMDIR"]))	
validationDir   <- trimws(paste(parameters$Value[parameters$Key=="VALID_DIR"]))	
# popSimDir       <- "E:/Projects/Clients/TulareCOG/Tasks/PopulationSim/Setup"
# validationDir   <- "E:/Projects/Clients/TulareCOG/Tasks/PopulationSim/Setup/validation"

scenario        <- "Base"						    
region          <- "Nashville" 

geographyList   <- c( "REGION","PUMA", "TAZ", "MAZ")
...
#for Nashville only - set PRMSE and SDEV to 0 as there is only one region
...
uniformity <- seed_households[seed_households$WGTP>0, seed_col] %>%
  ...
  left_join(summary_hhid, by = c("hh_id" = "hh_id")) %>%
